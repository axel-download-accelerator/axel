 ...
 */
static unsigned calc_percentage(off_t cur, off_t total) {
...
/* Set up internationalization (i18n) */
  ...
  while (1) {
    int option = getopt_long(argc, argv,
                             "s:n:o:S::46NqvhVapkcH:U:T:", axel_options, NULL);
    if (option == -1)
      break;

    switch (option) {
    case 'U':
      conf_hdr_make(conf->add_header[HDR_USER_AGENT], "User-Agent", optarg);
      break;
    case 'H':
      if (!(conf->add_header_count < MAX_ADD_HEADERS)) {
        fprintf(stderr,
                _("Too many custom headers (-H)! Currently only %u custom "
  ...
  if (argc - optind == 0) {
    ...
    if (strlen(s) > MAX_STRING) {
      fprintf(stderr, _("Can't handle URLs of length over %zu\n"), MAX_STRING);
      goto free_conf;
    }
  }
  ...
  if (do_search) {
    ...
    for (int i = 0; i < argc - optind; i++) {
      strlcpy(search[i].url, argv[optind + i], sizeof(search[i].url));
  ...
  if (*fn) {
    ...
    if (access(statefn, F_OK) == 0 && access(fn, F_OK) != 0) {
      printf(_("State file found, but no downloaded data. Starting from "
               "scratch.\n"));
      unlink(statefn);
    }
    ...
    for (int i = 0; 1; i++) {
      char statefn[MAX_STRING + 3];
      snprintf(statefn, sizeof(statefn), "%s.st", axel->filename);
      ...
      if (f_exists) {
        ...
        break;
      snprintf(s, axel->filename + sizeof(axel->filename) - s, ".%i", i);
    }
  }
 ...
 */
static inline unsigned log2i(unsigned long long x) {
...
char *axel_size_human(char *dst, size_t len, size_t value) {
  ...
  const unsigned int order =
      min(sizeof(oname) / sizeof(oname[0]) - 1, log2i(fval) / 10);
...
char *time_human(char *dst, size_t len, unsigned int value) {
  ...
  int ret;
  if (hh)
    ret = snprintf(dst, len, _("%u:%02u:%02u hour(s)"), hh, mm, ss);
  else if (mm)
    ret = snprintf(dst, len, _("%u:%02u minute(s)"), mm, ss);
  else
    ret = snprintf(dst, len, _("%u second(s)"), ss);
  ...
  for (off_t i = prev; i < cur; i++) {
    if (i % 50 == 0) {
      ...
      if (total == LLONG_MAX)
        ...
        printf("\n[%3u%%]  ", calc_percentage(1024 * i, total));
    } else if (i % 10 == 0) {
      putchar(' ');
    }
    putchar('.');
...
static void print_alternate_output_progress(axel_t *axel, char *progress,
                                            int width, off_t done, off_t total,
                                            double now) {
  if (!width)
    width = 1;
  if (!total)
  ...
  for (int i = 0; i < axel->conf->num_connections; i++) {
    ...
    if (axel->conn[i].currentbyte < axel->conn[i].lastbyte) {
      if (now <=
          axel->conn[i].last_transfer + axel->conf->connection_timeout / 2) {
...
void print_help(void) {
  ...
  printf(_(
      ...
      "\n"
      "Visit https://github.com/axel-download-accelerator/axel/issues to "
      "report bugs\n"));
#endif
}

void print_version(void) { printf(_("Axel %s (%s)\n"), VERSION, ARCH); }
